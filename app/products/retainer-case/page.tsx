            ...
            <div className="flex space-x-8 py-4 whitespace-nowrap">
              ...
              {categories.map(cat => (
                <Link key={cat.slug} href={`/products/${cat.slug}`} 
                  className={`uppercase pb-4 -mb-4 border-b-2 transition-all ${cat.slug === "retainer-case" ? "text-blue-600 border-blue-600" : "text-gray-500 border-transparent hover:text-blue-600"}`}>
                  {cat.name}
                </Link>
              ))}
            </div>
        ...
        <section className="py-16 px-4 max-w-7xl mx-auto">
          {items.length > 0 ? (
            ...
            <div className="text-center py-20 bg-white rounded-3xl border border-dashed border-gray-300">
              <p className="text-gray-400">Coming soon. Stay tuned for our new Retainer Cases collection.</p>
            </div>
          )}
        </section>
      </main>
