      ...
      <main className="pt-24 bg-gray-50 min-h-screen">
        ...
        <section className="bg-white border-b border-gray-100 py-16 px-4">
          <div className="max-w-7xl mx-auto">
            ...
            <p className="text-gray-500 text-lg max-w-2xl">Elegant packaging solutions for dental kits and accessories.</p>
          </div>
        </section>
            ...
            <div className="flex space-x-8 py-4 whitespace-nowrap">
              ...
              {categories.map(cat => (
                <Link key={cat.slug} href={`/products/${cat.slug}`} 
                  className={`uppercase pb-4 -mb-4 border-b-2 transition-all ${cat.slug === "gift-box" ? "text-blue-600 border-blue-600" : "text-gray-500 border-transparent hover:text-blue-600"}`}>
                  {cat.name}
                </Link>
              ))}
            </div>
        ...
        <section className="py-16 px-4 max-w-7xl mx-auto">
          {items.length > 0 ? (
            ...
            <div className="text-center py-20 bg-white rounded-3xl border border-dashed border-gray-300">
              <p className="text-gray-400">Coming soon. Stay tuned for our new Gift Boxes collection.</p>
            </div>
          )}
        </section>
      </main>
