      ...
      <main className="pt-24 bg-gray-50 min-h-screen">
        <section className="bg-white border-b border-gray-100 py-16 px-4">
          <div className="max-w-7xl mx-auto">
            <h1 className="text-3xl md:text-5xl font-bold mb-4 text-blue-600">Gift Boxes</h1>
            <p className="text-gray-500 text-lg max-w-2xl">Premium orthodontic and dental gift packaging. Elevate patient experience with custom-branded presentation boxes.</p>
          </div>
        </section>
        ...
        <section className="sticky top-20 z-40 bg-white border-b border-gray-200">
          <div className="max-w-7xl mx-auto px-4 overflow-x-auto text-sm font-bold">
            <div className="flex space-x-8 py-4 whitespace-nowrap">
              ...
              {categories.map(cat => (
                <Link key={cat.slug} href={`/products/${cat.slug}`} className={cat.slug === "gift-box" ? "text-blue-600 border-b-2 border-blue-600 pb-4 -mb-4 uppercase" : "text-gray-500 hover:text-blue-600 transition-colors uppercase"}>{cat.name}</Link>
              ))}
            </div>
          </div>
        </section>
        ...
        <CategoryDescription categoryKey="gift-box" />
      </main>
      <Footer />
    </>
  );
